...
(function () {
    ...
    MILTextExpander.directive('textExpander', function () {
        ...
        return {
            ...
            template: "<style>.titleText{margin-bottom: 25px;}.titleText span:last-child{float: right;}.expandButton{height: auto;-webkit-transition: all ease 0.3s;transition: all ease 0.3s;}.detailsText{}.rotateButton{-webkit-transform: rotate({{buttonRotation}}deg);transform: rotate({{buttonRotation}}deg);}.hideAnimation.ng-hide-add, .hideAnimation.ng-hide-remove{-webkit-transition: all ease 0.6s;transition: all ease 0.6s;}.hideAnimation.ng-hide-add{max-height: 300px;overflow: hidden;}.hideAnimation.ng-hide-add-active{max-height: 0px;}.hideAnimation.ng-hide-remove{max-height: 0px;overflow: hidden;}.hideAnimation.ng-hide-remove-active{max-height: 300px;}</style><div class='titleText' ng-click='displayText = !displayText'><span>{{title}}</span><span><img class='expandButton' src='{{buttonImage}}' ng-class='{rotateButton: displayText}'></span></div><div class='detailsText hideAnimation' ng-show='displayText'>{{text}}</div>",
